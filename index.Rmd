...
This tutorial is laid out as specific tasks.
Throughout the tutorial you may notice some dependencies which are from tasks further down the document, this is because the chronology of the tutorial is not the same as how it was made.
For each task, you will find a short list of the required tools.
Installation instructions have been provided however, I recommend using the exact Conda environments I generated - as these have already been tested.
You can find the environment YML files on the GitHub repository <https://github.com/harrychown/assembly-annotation>.
There you will also find instructions on how to install the Conda environment using the appropriate files.

For each tool used, I provide a one sentence description of its task, the version number and links to the relevant papers/wiki/GitHub page.

I also provide a line-by-line run through of how the script works.
Take in mind that these have been developed for use on the Imperial College London HPC, therefore, it may not fully transfer to your system.
Exact code can be copied here or files can be found on the GitHub page: <https://github.com/harrychown/assembly-annotation/tree/main/scripts>.
...
Mitochondrial genome of *A. fumigatus* strain A1163. Accession: [NC_017016](https://www.ncbi.nlm.nih.gov/nuccore/NC_017016). Members of FisherLab can find data at:
```
~/../projects/fisher-aspergillus-reference/live/NC_017016.1_mito.fa
```

### Reference genome for scaffolding
The genome that we use for scaffolding is from *A. fumigatus* strain CEA10. Accession: [GCA_051225625.1](https://www.ncbi.nlm.nih.gov/datasets/genome/GCA_051225625.1/).
Members of FisherLab can find data at:
```
~/../projects/fisher-aspergillus-reference/live/CEA10.fasta
```
...
* FastP - filter and trim reads
  ...
  + [Paper](https://doi.org/10.1002/imt2.70078) 

Installation:
```
conda create -n short-qc bioconda::fastp bioconda::fastqc 
```
Script for short-read QC analysis can be found at:
```
~/../projects/fisher-aspergillus-analysis/live/tutorial/assembly-annotation/scripts/complete/short_qc.job
```
Usage:
```bash {.numberLines}
#!/bin/bash
...
PREFIX=UU
OUTDIR=~/../projects/fisher-aspergillus-analysis/live/tutorial/assembly-annotation/data
#--------------------------------------------
# 3. Pre-trim QC
#--------------------------------------------
cd $TMPDIR
mkdir -p $OUTDIR/short_read
conda activate short-qc
fastqc -o $OUTDIR/short_read $R1
fastqc -o $OUTDIR/short_read $R2
#--------------------------------------------
# 4. Trim
#--------------------------------------------
fastp -i $R1  -I $R2  -o $OUTDIR/short_read/${PREFIX}_1.trimmed.fastq.gz  -O $OUTDIR/short_read/${PREFIX}_2.trimmed.fastq.gz
#--------------------------------------------
# 5. Post-trim QC
#--------------------------------------------
# Re-assess trimmed quality - fastqc
fastqc -o $OUTDIR/short_read $OUTDIR/short_read/${PREFIX}_1.trimmed.fastq.gz 
fastqc -o $OUTDIR/short_read $OUTDIR/short_read/${PREFIX}_2.trimmed.fastq.gz
```
**Lines 20-23** assess the quality of pre-trimmed sequences

**Line 27** trims the adapter sequences and removes low-quality reads

**Lines 32-33** re-assess the quality after trimming
...
* GetOrganelle - organelle identification
  ...
  + [Paper](https://doi.org/10.1186/s13059-020-02154-5)
* BEDTools - extract reads from alignment file
  + Version: 2.31.1
  + [GitHub](https://github.com/arq5x/bedtools2) 
  + [Paper](https://doi.org/10.1093/bioinformatics/btq033)
  
We will also use tools to align and extract mitochondrial reads.
The alignment steps follow the polishing instructions below so please see those details if you require further information on the tools (minimap2, SAMtools).

  
Installation:
```
conda create -n getorganelle bioconda::getorganelle bioconda::bedtools
```
If you have not previously ran polishing (see below), you will also need to create this environment:
```
conda create -n polish bioconda::minimap2 bioconda::samtools
```
Script for mitochondrial assembly and read extraction can be found at:
```
~/../projects/fisher-aspergillus-analysis/live/tutorial/assembly-annotation/scripts/complete/mitochondria.job
```
...
mv *.fastq.gz $OUTDIR
```
**Lines 233-246** assemble mitochondria using a reference for scaffolding

**Lines 252-255** map short-reads to assembly and extracts unmapped (nuclear) alignments
**Lines 258-260** converts alignment files to FASTQ

# Assembly

## Short-read assembly

We are going to start by generating an initial assembly, using SPAdes.
The reads we will be using in this example have been labelled as nuclear-derived after generating mitochondrial genomes (see above).
Then, we will scaffold the resulting contigs/scaffodls against a reference genome.
This will help improve the order of contigs.
Since we have short-read data, we will re-map the short reads to the scaffold and try to improve any local misassemblies.

Tools for initial assembly and scaffolding:

* SPAdes - assembler
  + Version: 4.2.0
  + [GitHub](https://github.com/Kinggerm/GetOrganelle) 
  + [Paper](https://doi.org/10.1186/s13059-020-02154-5)
* AAFTF (sourpurge step) - contamination removal
  + Version: 0.6.0
  + [GitHub](https://github.com/Kinggerm/GetOrganelle) 
  + [Paper](https://doi.org/10.1186/s13059-020-02154-5)
* Ragtag - scaffolder
...
We will also use tools to align and improve the assembly.
The alignment and polishing steps follow the polishing instructions below so please see those details if you require further information on the tools (minimap2, SAMtools, Pilon).

  
Installation:
```
conda create -n short-asm -c bioconda "python>=3.6" bbmap trimmomatic bowtie2 bwa pilon sourmash \
    blast minimap2 spades megahit novoplasty biopython fastp masurca unicycler ragtag
conda activate short-asm
python -m pip install git+https://github.com/stajichlab/AAFTF.git
```
For FL members, a sourmash database has been preinstalled. 
For non-FL members you will need to install the appropriate database like so:
```
cd ~/../projects/fisher-local-software/live/AAFTF_DB
wget https://osf.io/4f8n3/download/genbank-k31.lca.json.gz
```
If you have not previously ran polishing (see below), you will also need to create this environment:
```
conda create -n polish bioconda::minimap2 bioconda::samtools
```
We will also be using a local install of Pilon.
Please see the ONT polishing steps for details on installation of Pilon on your machine.

Script for short-read assembly and refinement can be found at:
```
~/../projects/fisher-aspergillus-analysis/live/tutorial/assembly-annotation/scripts/complete/short_asm.job
```
...
```
**Lines 25-31** generate an initial assembly which we then improve upon

**Lines 36-40** check contigs for artefacts arising from contamination

**Lines 44-48** scaffolds the contigs against a reference assembly

**Lines 52-68** re-aligns reads to the assembly to help refine the assemby.
This is carried out 5 times
...
**Line 26** organises the contigs/scaffolds in descending order and renames them as scf_1, scf_2...scf_N
...
* EarlGrey - TE annotater
  + Version: 7.0.1
  + [Paper](https://doi.org/10.1093/molbev/msae068) 
...
An example of this has been uploaded to <https://github.com/harrychown/assembly-annotation/tree/main/scripts>.
...
**Line 23** runs EarlGrey using repeats from Fungi found in Dfam database.
Options ```-c``` clusters redundant TEs, ```-d``` generates soft-masked genomes at the output and ```-e``` detects Helitrons.
